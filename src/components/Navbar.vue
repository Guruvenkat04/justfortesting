<template>
  <nav :class="className" :is="tag" :style="navStyle" v-if = "enableValue == 0" >
    <a href="#/" class="navbar-brand" >{{name}}
      <img src="../assets/justopslogo.svg" style="width: 15%;">
    </a>
    <!-- <a :href="href" class="navbar-brand" v-if="header2 == 1" >{{name}}
      <img src="../assets/justopswhite.png" style="width: 15%;">
    </a> -->
  ...
  <nav :class="className" :is="tag" :style="navStyle" v-else>
    <a href="#/" class="navbar-brand1">{{name}}
      <img src="../assets/justopswhite.png" style="width: 15%;">
    </a>
    <button class="navbar-toggler" type="button" data-toggle="collapse" :data-target="target" aria-controls="navbarSupportedContent"
        aria-expanded="false" aria-label="Toggle navigation">
...
export default {
  ...
  mounted() {
    this.enableValue = localStorage.getItem('sonarUrl');  
